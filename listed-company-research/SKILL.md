...
**Priority 0.5 — CIQ Financial Files (Google Sheets + Local Excel):**
The user maintains CIQ-sourced financial data in Google Sheets and **two types of local files**. Check local files first (faster, no API call), then fall back to Google Sheets.

- **Type A — Google Sheet Exports (`.xlsx`):** `/home/mike/hermes-workspace/装修建材.xlsx` (建材/家居/水泥). Multi-tab format, one tab per company. Use `openpyxl`.
- **Type B — CIQ Financials Directory (`.xls`):** `/home/mike/hermes-workspace/CIQ_Financials/` — individually exported CIQ files, one per company. Old `.xls` format — use `xlrd` NOT `openpyxl`. Contains 13 sheets (Income Statement, Balance Sheet, Cash Flow, Ratios, Segments, etc.) with **19+ years** of data. Far richer than Type A for BS detail, CF breakdown, and segment analysis. Always check this directory FIRST for long-cycle financial history.
- **Google Sheets:** Multiple industry-specific spreadsheets. Use the `google-workspace` skill.

See `references/gs-financial-structure.md` for column/row mapping, extraction patterns, and known file inventory.

**Pitfall — multi-source discovery:** Do NOT hard-code a single source. If the company is not found in Type A or Type B, search for others: `find /home/mike/hermes-workspace/CIQ_Financials -name "*.xls" -type f` and `find /home/mike -name "*.xlsx" -type f | grep -v node_modules`. The user adds new CIQ exports over time.
